...
@section('content')
<div  ng-app="EditPOApp" ng-controller="EditPOController" ng-cloak>
    <div class="card" ng-init="editPurchaseOrder({{$id}}); po.id={{$id}};">
        <div class="card-body">
...
<script>
    var EditPO = angular.module('EditPOApp', [], function ($interpolateProvider) {
        $interpolateProvider.startSymbol('<%');
        $interpolateProvider.endSymbol('%>');
    });

    EditPO.directive('datepicker', function () {
        return {
            restrict: 'A',
            require: 'ngModel',
            compile: function () {
...
//                            scope[attrs.ngModel] = dateObj.getFullYear() + '-' + (dateObj.getMonth() + 1) + '-' + dateObj.getDate();
    ...
    EditPO.controller('EditPOController', function ($scope, $http) {
        $scope.po = {};
        ...
        $scope.getVendors = function (ven_id) {
            $scope.vendorinfo = {};
            $http.get($scope.appurl + 'vendor/get-vendor/' + ven_id).then(function (response) {
                if (response.data.length > 0) {
        ...
        $scope.getInventoryInfo = function(){
            $scope.products = {};
            $http.get($scope.appurl + 'get-inventory').then(function (response) {
                if (response.data.length > 0) {
        ...
        $scope.getProductInfo = function(pro_id){
            $http.get($scope.appurl + 'get_pro_info/' + pro_id).then(function (response){
        ...
        $scope.savePurchaseOrder = function(){
            ...
            if (!$scope.po.vendor_id) {
                ...
                $http.post($scope.appurl + 'save-purchase-order', Data, {transformRequest: angular.identity, headers: {'Content-Type': undefined}}).then(function (res) {
                    swal({
                        title: "Save!",
                        text: res.data,
                        type: "success"
        ...
        $scope.editPurchaseOrder = function (id) {
            $http.get($scope.appurl + 'edit-purchaseorder/' +id).then(function (response) {
                $scope.po = response.data;
                $scope.po.vendor_id = parseInt(response.data.vendor_id);
                $scope.po.chartofaccount_payment = parseInt(response.data.chartofaccount_payment);
                $scope.po.chartofaccount_purchase = parseInt(response.data.chartofaccount_purchase);
                $scope.po.product_id = parseInt(response.data.product_id);
                $scope.editProductInfo($scope.po.id);
                //console.log($scope.inventory);
            });
        };

        $scope.editProductInfo = function(po_id){
            $http.get($scope.appurl + 'edit_pro_info/' + po_id).then(function (response){
                $scope.allproducts = response.data;
                //angular.extend($scope.po, response.data);
            });
        };
