...
class CompanyBudgetController extends Controller
{
    ...
    public function saveBudget(Request $request)
    {
        //return $request->all();
        if($request->id){
            $data = $request->except(['account_id', 'company_id', 'CategoryName', 'created_at', 'updated_at', '$$hashKey', 'id']);
            ...
            $data = $request->all();
            $srch = ErpCompanyBudgets::where('account_id', $request->account_id)->where('year', $request->year)->first();
            if(empty($srch)){
    ...
    public function searchBudgetDetail($search)
    {
        $budget = DB::select('SELECT category.CategoryName, budget.* FROM (SELECT * FROM erp_company_budgets) AS budget join(SELECT id, CategoryName FROM tblaccountcategories WHERE CategoryName LIKE "%'.$search.'%") AS category ON category.id = budget.account_id');
        $newbudget = array();
        foreach ($budget as $key => $value) {
            //echo $value->july;exit();
            $value->july = (float)$value->july;
